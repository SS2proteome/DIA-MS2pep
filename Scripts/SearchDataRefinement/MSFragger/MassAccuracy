...
sub EM_iterator{
	...
	map{
		my $i = $_;
		my $post_f = post_function(normal_function($last_ave,$last_std,$i),$prob1_,$prob_x_w2,$prob2_);
		$temp1 += $post_f;
		$temp2 += $post_f * $i; # sum of p(w1|x)*x

		$temp3 += $post_f *(($i-$last_ave)*($i-$last_ave)); # sum of p(w1|x)*(x-s)^2
	}@{$data};
	...
	while((abs($last_ave-$current_ave) + abs($last_std-$current_std)) >= $limit){
		map{
			...
			next if $post_f < 0 ;
			$temp1 += $post_f;
			$temp2 += $post_f * $_; # sum of p(w1|x)*x
			$temp3 += $post_f *(($i-$last_ave)*($i-$last_ave)); # sum of p(w1|x)*(x-s)^2
		}@{$data};
