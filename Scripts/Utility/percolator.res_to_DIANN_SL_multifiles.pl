...
if(scalar @mgfs == 0){
@mgfs = glob "*Calmz_23aug2017_hela_serum_timecourse_wide*.mgf";#"Hela1ug_QC_Middle_DDA_150226_01.mgf";
}
#@mgfs = glob "Calmz_${filename}*mgf" if ! (scalar @mgfs);#"Hela1ug_QC_Middle_DDA_150226_01.mgf";
#my @pinfiles = glob "${filename}*pin";# *pin
my @percolatorfiles = @ARGV; #glob "Hela1ug_QC_Middle_DIA_[0-9]*.pin.target.pep.tsv";# *target.pep.tsv
my $qvalue = 0.01;
...
open(OUT,">${filename}_DIANN_Lib.tsv");
...
while(<DIAMGF>){
	...
	if(/^TITLE=(.*)/){
		...
		$tag = 0;
		undef @ion_arr;
		if(exists $list_title{$title} and !$unql{$title}++){
	...
	if(/END/ && $tag){
		...
		$pep =~ s/([STY])\[[^][]+\]/$1(UniMod:21)/g;
		foreach my $i (0..$#$matched_mzs_type){
			my ($SeriesNumber,$FragmentType,$FragmentCharge) = ($matched_mzs_type->[$i] =~ /(\d+)\.([by][^+]*)([+]+)/);
			if($SeriesNumber == 1){next;}
			my $loss = $FragmentType =~ /H2O/ ? "H2O" : $FragmentType =~ /NH3/ ? "NH3" : $FragmentType =~ /H3PO4/ ? "H3PO4" : "noloss"; 
			$FragmentType =~ s/_(H2O|NH3|H3PO4)//;
