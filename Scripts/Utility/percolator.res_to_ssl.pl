...
my $aa_mass = AA_mass();
open(PIN,$filename.".pin.target.pep.tsv") or die "$filename";
open(OUT,">".$ssl_file) or die "$ssl_file";
print OUT join "\t",qw/file   scan    charge  sequence PTM /;
print OUT "\n";
while(<PIN>){
	chomp;
	s/\r//;
	...
	if($line[2] < 0.01){
		...
		print OUT join "\t",$file,$scan,$charge,$sequence,$ptm;
		print OUT "\n";
	}
}
close(PIN);
