...
my $cycle;
my $experiment;
open(mzML,"$mzmlfile") or die "No $mzmlfile $!\n";
while(<mzML>){
	if(m{<spectrum index="([^"]+)" id="sample=\d+ period=\d+ cycle=(\d+) experiment=(\d+)"}){
		($index,$cycle,$experiment) = ($1,$2,$3);
		$index_hash->{$index} = [$cycle,$experiment];
		print "\r$index,$cycle,$experiment";
	}
}
close(mzML);
print "\n";
...
print OUT join "\t","Raw File","Stripped Sequence", "Precursor Charge", "Labeled Sequence", "Retention Time","Scan Number","Scan Event";
print OUT "\n";
while(<PIN>){
	chomp;
	s/\r//;
	...
	if($line[2] < 0.01){
		...
		print OUT join "\t",$file,$stripped_seq,$charge,$sequence,$rt_data->{$line[0]},$index_hash->{$scan}->[0],$index_hash->{$scan}->[1]-1;
		print OUT "\n";
	}
}
close(PIN);
