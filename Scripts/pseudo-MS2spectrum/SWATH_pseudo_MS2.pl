...
my $num_of_data_points = 3; #@ARGV[5];
#my $min_pearson = $ARGV[4]; #@ARGV[5];
...
#print Dumper $scan_list_index;


my $min_charge = 1;
my $max_charge = 5;
#my $num_of_data_points = @ARGV[5];
my $ms2scan;
my %SWATH_window;
...
#  READ MGF FILE
print "\n\n*****Read mgf_file file: $mgf_file and assaign the charge into mgf!\n";
my $title_line;
my $rt_line;
my $pepmass_line;
my $ion_lines;
...
while(<FH>){
	chomp;
	#if(/^TITLE=sample=[^ ]+ period=[^ ]+ cycle=(\d+) experiment=(\d+)/){#TITLE=Site10_DynRge_S1_Day_1.1.1.2
	if(/^TITLE=([^.]+).([^.]+).([^.]+).([^.]+)/){#TITLE=Site10_DynRge_S1_Day_1.1.1.2
		...
		#){
			#next if $cnt < 30 ;
			# next if $ms2scan < 30000;
			# exit if $ms2scan > 32000;			
			...
			if($buffer1_ms2scan_data->{$premz}->[0]){
				...
				# noise filter start
				my $com_mz;
				# #print Dumper $curr_ms1scan_data,$pre_ms1scan_data,$next_ms1scan_data;
				# $com_mz = Spectrum_cmp($buffer2_ms2scan_low_signal_data->{$premz},$buffer1_ms2scan_data->{$premz},$com_mz);
				# $com_mz = Spectrum_cmp($buffer2_ms2scan_low_signal_data->{$premz},$buffer2_ms2scan_low_signal_data->{$premz},$com_mz);
				# $com_mz = Spectrum_cmp($buffer2_ms2scan_low_signal_data->{$premz},$buffer3_ms2scan_data->{$premz},$com_mz);
			    
				#print "\traw:  \t",scalar @{$buffer2_ms2scan_data->{$premz}}, "\n";
				foreach my $ref (@{$buffer2_ms2scan_data->{$premz}}){
					my ($mz,$i) = @{$ref};
					#if(defined $com_mz->{$mz}){
					# if(0){
						# my @ints = (@{$com_mz->{$mz}});
						# #print join "\t",$ints[0],$ints[2],"\n";
						# if($ints[0] > 0 || $ints[2] > 0){
							# push @{$filtered_ion_arr->{$premz}},[$mz,$ints[1]];
							# $noise_ = $noise_ > $ints[1] ? $ints[1] : $noise_;
						# }
					# }else{
						push @{$filtered_ion_arr->{$premz}},$ref;
					# }
				}
				#print "filter:  \t",scalar @{$filtered_ion_arr->{$premz}}, "\n" if (defined $filtered_ion_arr->{$premz});
				# noise filter end
				#push @{$filtered_ion_arr->{$premz}};
			}
			
			#my ($filtered_ion_arr,$noise_) = [@peaks];
			#print join "\t",($#$filtered_ion_arr,$noise_,"---\n");
			if (defined $filtered_ion_arr->{$premz}){
...
sub boundary_index{
	...
	$start = defined $start ? $start+1 : 0;
    $end = defined $end ? $end-1 : $#$data;
